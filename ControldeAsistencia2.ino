...
const char* UBIDOTS_TOKEN = "BBFF-9PTQHJj6oTkNLMh4IX6HA4lKsJK2ms";  // Put here your Ubidots TOKEN
const char* WIFI_SSID = "Casa";      // Put here your Wi-Fi SSID
const char* WIFI_PASS = "86269984SSJJ";      // Put here your Wi-Fi password
...
int servo = 0;
int runn = 0;
const int trigPin =  12;  // pin del trig del ultrasonico.
const int echoPin =  16; // pin de echo del ultrasonico.
long duration;
int distance;
int menu = 0;
...
void setup() {
  ...
  lcd.print(" ASIS IA "); // TEXTO DE INICIO
  mlx.begin();
...
static inline int do_enrollment(face_id_name_list *face_list, dl_matrix3d_t *new_id)
{
  ...
  ESP_LOGD(TAG, "ID DE ROSTRO %s AJUSTANDO: Sample %d",
           st_name.enroll_name,
...
void open_door(WebsocketsClient &client) {
  if (runn = 0) {
    Serial.println("ASISTENCIA TOMADA");
    client.send("rostro_ok");
...
void loop() {
  ...
  while (client.available()) {
    ...
    if (g_state == START_DETECT || g_state == START_ENROLL || g_state == START_RECOGNITION)
    {
      ...
      if (out_res.net_boxes)
      {
        if (align_face(out_res.net_boxes, image_matrix, aligned_face) == ESP_OK)
        {
          ...
          if (g_state == START_ENROLL)
          {
            ...
            sprintf(enrolling_message, "MUESTRA %d NUMERO %s", ENROLL_CONFIRM_TIMES - left_sample_face, st_name.enroll_name);
          ...
          if (g_state == START_RECOGNITION  && (st_face_list.count > 0))
          {
            ...
            if (f)
            {
              ...
              sprintf(recognised_message, "ASISTENCIA REGISTRADA %s", f->id_name);
              open_door(client);
              ubidots.add(f->id_name, t);
      ...
      {
        if (g_state != START_DETECT) {
          client.send("NINGUN ROSTRO DETECTADO");
        }
      }
    ...
    if (sensor == 1 && acceso == 0) {

      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("Acerquese para");
      lcd.setCursor(1, 1);
      lcd.print("Ver su rostro");
      g_state = START_RECOGNITION;
      client.send("RECOGNISING");
      sensor = 2;
      acceso = 2;
    ...
    if (sensor == 3 && acceso == 3) {

      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print(" Colocar Su");
      lcd.setCursor(1, 1);
      lcd.print("Frente o brazo");
      h = mlx.readAmbientTempC();
      t = mlx.readObjectTempC();
      h = mlx.readAmbientTempC();
      t = mlx.readObjectTempC();
      ...
      if (t >= 37.2) {
        ...
        ubidots.add("Alerta", 1);
        bool bufferSent = false;
        bufferSent = ubidots.send();
        if (bufferSent) {
          Serial.print("Tiene Fiebre: ");
          Serial.println(t);
          //sensor = 0;
          distance = 200;
          acceso = 4;
